...
proc freq data=rawdata0;
tables cancer_2digit;
run;

data rawdata;
set rawdata0;
where (cancer=1 and cancer_2digit="C34") or cancer=0;
run;
...
PROC EXPORT DATA= WORK.DATA_FOR_MATCHING 
            OUTFILE= "Z:\8 GQ Zhang group\STEP1 data analysis\Before_matching_lung_cancer.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN; 
...
PROC IMPORT OUT= WORK.after_match
            DATAFILE= "Z:\8 GQ Zhang group\STEP1 data analysis\matched_cancer_geq_18_lung_cancer.csv" 
            DBMS=CSV REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
RUN;
...
ods rtf file="Z:\8 GQ Zhang group\STEP1 data analysis\OUTPUT\All tables_lung cancer.rtf" startpage=never;
...
PROC EXPORT DATA= WORK.alltime  
            OUTFILE= "Z:\8 GQ Zhang group\STEP1 data analysis\alltime_lung_cancer.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
PROC EXPORT DATA= WORK.matched_data  
            OUTFILE= "Z:\8 GQ Zhang group\STEP1 data analysis\matched_data_lung_cancer.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
ods rtf file="Waste.rtf" startpage=never;
ods text="death";
proc print data=B_1; run;
ods text="inpatient";
proc print data=B_2; run;
