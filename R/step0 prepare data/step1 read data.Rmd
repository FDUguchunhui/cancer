...
```{r}
save(rawdata0, file = 'data/rawdata0.RData')
```
...
rawdata1 <- rawdata0 %>%
  ...
  mutate(race_gp = ifelse(ethnicity == 'Not Hispanic' && race == "Caucasian", 'Non-hispanic white', 
                          ifelse(ethnicity == 'Not Hispanic' && race == "African American", 'Non-hispanic black',
                                 ifelse(ethnicity == 'Hispanic', 'Hispanic', 'Others/Unknown')
                                 )
                          )
         ) 
```
...
rawdata1 <- rawdata1 %>%
  rowwise() %>% 
  mutate(smoke_status = ifelse(is.na(smoke), 'd:Other/unknown', 
                                                      ifelse(smoke == 'Current smoker', 'a:current',
                                                        ifelse(smoke == 'Previously smoked', 'b:former',
                                                             ifelse(smoke %in% c('N', 'Never smoked'), 'c:never',
                                                                    ifelse(smoke %in% c("Not recorded", "Other smoking status","Not currently smoking","Unknown smoking status"), 'd:Other/unknown', NA)))))
                                )
```
...
rawdata1 <- rawdata1 %>%
  rowwise() %>% 
  mutate(smoke_status = ifelse(smoke %in% c("Not recorded", "Other smoking status","Not currently smoking","Unknown smoking status", NA) && smoke_by_icd == 1 && smoke_by_icd_group == 'current', "a:current",
                               ifelse(smoke %in% c("Not recorded", "Other smoking status","Not currently smoking","Unknown smoking status", NA) && smoke_by_icd == 1 && smoke_by_icd_group == 'former', 'b:former', smoke_status)
                               )
         ) 
```
...
rawdata1 <- rawdata1 %>% 
  rowwise() %>% 
  mutate(smoke_yn = ifelse(smoke_status %in% c("a:current", "b:former"), 1, 
                           ifelse(smoke_status != "d:Other/unknown", 0, NA)))
```
...
rawdata1 <- rawdata1 %>% 
  rowwise() %>% 
  mutate(severity = ifelse((death == 1 || inpatient == 1 || icu == 1 || ventilation == 1), 1, 0))
...
rawdata1 <- rawdata1 %>% 
  rowwise() %>% 
  mutate(event_count = death + inpatient + icu + ventilation,
