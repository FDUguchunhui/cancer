...
rawdata4 <- rawdata4 %>% 
  ...
  mutate(
         age_at_cancer = (cancer_year - birth_yr),
         duration = as.numeric(covid_pcr_date - cancer_date)/365.25,
...
rawdata5 <- rawdata5 %>% 
  # rowwise() %>% 
  mutate(death_month = month(date_of_death),
         covid_month = month(`covid diag date`),
         month_to_death = death_month - covid_month,
         # if death happen before the month of diag then treated as missing
         death_on_month_0 = ifelse(is.na(month_to_death), 0,
                                   ifelse(month_to_death == 0, 1, 0))
         # not need to calculate survival more than 1month 
         # month_to_death = ifelse(month_to_death < 0, NA, month_to_death),
...
proc_freq(rawdata7, c('death', 'severity_level', 'severity'))
```
